package com.dataservicios.ttauditalicorppreventagalleta.repo;

import android.content.Context;

import com.dataservicios.ttauditalicorppreventagalleta.db.DatabaseHelper;
...
public class PollRepo implements Crud {
    ...
    public Object findByOrder(int order) {

        Poll wishList = null;
        try {
            wishList =  helper.getPollDao().queryBuilder().where().eq("order",order).queryForFirst();
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return wishList;
